...
-- 2. Restrict eSIM plans access - only show retail pricing to agents
...
-- 3. Enhance wallet transactions security - restrict service role access
DROP POLICY IF EXISTS "Service role can select wallet transactions" ON public.wallet_transactions;

-- More restrictive service role policy for wallet transactions
CREATE POLICY "Service role can insert wallet transactions" 
ON public.wallet_transactions 
FOR INSERT 
WITH CHECK (auth.jwt() ->> 'role' = 'service_role');

-- 4. Enhance eSIM topups security - add validation
