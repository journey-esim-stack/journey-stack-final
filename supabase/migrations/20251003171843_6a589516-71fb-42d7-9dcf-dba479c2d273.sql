...
-- Only admins and service role can access esim_plans table
...
-- Step 6: Update orders RLS policy to be more restrictive for direct table access
...
CREATE POLICY "Only admins can view orders table with wholesale data"
ON public.orders
FOR SELECT
TO authenticated
USING (
...
-- Step 7: Comment to document the security architecture
COMMENT ON VIEW public.agent_safe_plans IS 'Secure view for agents - excludes wholesale_price, supplier_name, supplier_plan_id';
COMMENT ON VIEW public.agent_safe_orders IS 'Secure view for agents - excludes wholesale_price, supplier_order_id';